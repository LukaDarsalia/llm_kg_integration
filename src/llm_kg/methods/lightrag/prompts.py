"""Prompts ported verbatim from LightRAG's `lightrag/prompt.py` (HKUDS/LightRAG @ main).

Kept as a single PROMPTS dict so future updates can be diffed against upstream
trivially. Format-string placeholders (e.g. {tuple_delimiter}, {language})
are filled in by the LightRAG stage classes.
"""
...
3. Output:
  - Output ONLY the final short answer string. Do not include any explanation, references, markdown headings, or commentary.
  - The answer MUST be in the same language as the user query.
...
# Verbatim port of LightRAG's upstream rag_response prompt (HKUDS/LightRAG main,
# lightrag/prompt.py lines 224-276). Targets long-form markdown answers with a
# references section. Useful for a controlled experiment: replacing our
# short-answer prompt with this one should drop EM/F1 dramatically and match
# the LightRAG numbers reported in AutoSchemaKG Table 5 / HippoRAG 2.
#
# NOTE: upstream uses {response_type}, {user_prompt}, {context_data} placeholders.
# We rename to {context} so it plugs into DefaultGenerator unchanged (which
# substitutes {context} and {question}), and hard-code response_type +
# empty user_prompt inline.
PROMPTS["rag_response_longform_upstream"] = """---Role---
